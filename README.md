...
# Load uptake and secretion rates
with open('../Data/Uptake_Secretion_Rates/WT_pkl_mat_excel/uptake_secretion_raw_wt_dict.pkl', 'rb') as file:
    uptsec_wt = pickle.load(file)
with open('../Data/Uptake_Secretion_Rates/ZeLa_pkl_mat_excel/uptake_secretion_raw_zela_dict.pkl', 'rb') as file:
    uptsec_zela = pickle.load(file)

# WT Growth rate predictions
results = []
...
for name,model in wt_models.items():
    ...
    for interval_key, interval_model_time in intervals.items():
        if interval_model_time in name:
            for rep_key, rep_suffix in replicates.items():
                if rep_suffix in name:
