function [tissueModel, cRes] = pruningModel_v2(model, rankNonCore, coreRxn, zeroExpRxns, precursorMets, eta, tol)
...
%          - Modified and commented by S. Opdam and A. Richelle,May 2017
    ...
    %fig1 = uifigure;
    %d1 = uiprogressdlg(fig1,'Title','Pruning Model using mCADRE','Message','Initializing...');
    n_noncore = numel(rankNonCore);
    while numel(rankNonCore) > 0
     ...
     %   display(['Reaction no. ', num2str(count)])
        ...
        % check reaction essentiality before removal
        m1 = changeObjective(tissueModel,r,1);
        sol = optimizeCbModel(m1,'max');
        vmax = sol.f;
        ...
        if rStatus && rmf_stat
            ...
            % Check for inactive reactions after removal of r
            [fluxConsistentMetBool,fluxConsistentRxnBool] = findFluxConsistentSubset(model_rem,paramConsistency,0);
        ...
        %d1.Value = 1 - numel(rankNonCore)/n_noncore;
        %d1.Message = [num2str(round(100*d1.Value,2)),'% Complete: ',num2str(num_removed),' reactions removed from the model']; 
    end
    %delete(d1);
    %delete(fig1);
    cRes(count:end) = [];
