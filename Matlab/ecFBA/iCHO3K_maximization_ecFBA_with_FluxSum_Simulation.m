        ...
        for i = 1:length(Models)
            ...
            for j = 1:length(Rxn_id)
                if model_rate(j)<0
                    ...
                    test_model = changeRxnBounds(test_model,Rxn_id(j),model_rate(j)*0.9,'u');
                else
                    test_model = changeRxnBounds(test_model,Rxn_id(j),model_rate(j)*0.9,'l');
                ...
                switch pick
                    case 1
                        ...
                        test_model = changeRxnBounds(test_model,'biomass_cho_s',0,'l');
                        test_model = changeRxnBounds(test_model,'biomass_cho_s',1000,'u');
                    ...
                    case 2
                        ...
                        if model_rate(20) ~=0
                            test_model = changeRxnBounds(test_model,'EX_lac_L_e',0,'l');
                            test_model = changeRxnBounds(test_model,'EX_lac_L_e',0,'u');
                            disp("l-Lactate uptake and secretion constrained w 10% error");
                        else
                ...
                fprintf('\n%s Maximization ecFBA Simulation Initiated ...\n',string(Model_id(i)));
                [maxEC_modelIrrev.(strcat("modelIrrev_",Model_id(i))), maxEC_Solutions.(strcat("ec_Solutions_",Model_id(i))),maxEC_fluxSum.(strcat("fluxSum_",Model_id(i)))] = maxEcFBAwithFluxSum(test_model,0.1,'biomass_cho_s',sample_size);
